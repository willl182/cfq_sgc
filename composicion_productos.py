...
# --- Cargar list.csv: BOM ---
bom_rows = []
with open("list.csv", newline="", encoding="utf-8-sig") as f:
    reader = csv.DictReader(f)
    for row in reader:
...
with open("composicion_productos_resultado.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
print(f"\n\nCSV guardado: composicion_productos_resultado.csv")
