...
DOSSIER_ROOT = PROJECT_ROOT / "SGC_CALFERQUIM" / "05_Dossier_Productos"
...
def main() -> None:
    ...
    for dossier_dir in sorted(path for path in DOSSIER_ROOT.iterdir() if path.is_dir() and not path.name.startswith("_")):
        ...
        if match.status.startswith("matched"):
            ...
            master_row["archivo_detalle"] = str(detail_path.relative_to(PROJECT_ROOT))
            master_row["archivo_resumen"] = str(summary_path.relative_to(PROJECT_ROOT))
            master_rows.append(master_row)
            continue
