...
var CATALOGO_MP_HEADERS = [
  'ID_PROD', 'COD', 'PRODUCTO', 'PROVEEDOR', 'Cprov', 'CLASE', 'TIPO', 'NOMBRE',
...
function handleSaveMP(body) {
  ...
  if (items.length > 0) {
    var rows = items.map(function (item) {
      return objectToRow(item, CATALOGO_MP_HEADERS);
    });
    sheet.getRange(2, 1, rows.length, CATALOGO_MP_HEADERS.length).setValues(rows);
  }
