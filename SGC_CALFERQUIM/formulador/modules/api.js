...
const DEFAULT_API_URL = window.FORMULADOR_API_URL || 'https://script.google.com/macros/s/AKfycbzlO8HjfoeQxClhA5Vm7jo6bwHHS_36yutp0nbjPDGRHRa5I1XC2tw3p0luB4wKtrw/exec';
...
export const Api = {
  ...
  setUrl(url) {
    ...
    localStorage.setItem('formulador_api_url', url);
  },
  ...
  getUrl() {
    if (!this._baseUrl) {
      this._baseUrl = localStorage.getItem('formulador_api_url') || DEFAULT_API_URL || '';
    }
  ...
  async fetchMP(forceRefresh = false) {
    ...
    if (!forceRefresh) {
      const cached = localStorage.getItem('formulador_cache_mp');
      if (cached) {
        try {
    ...
    localStorage.setItem('formulador_cache_mp', JSON.stringify(this._cacheMP));
    return this._cacheMP;
  },
  ...
  _postViaForm(endpoint, action, body = {}) {
    return new Promise((resolve, reject) => {
      const iframeName = `formulador_iframe_${Date.now()}`;
