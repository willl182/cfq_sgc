...
export const Catalogo = {
  ...
  async _loadData() {
    try {
      ...
      try {
        ...
        this._data = [];
        Utils.toast('No se pudo leer Sheets; usando CSV local', 'warning', 5000);
      }

      if (!this._data || this._data.length === 0) {
        this._data = await this._preloadFromBundledCsv();
      }
      this._applyFilters();
  ...
  async _handleImport(file) {
    ...
    try {
      ...
      const productos = this._mergeCatalogs(
        CsvParser.parseProductos(text),
        CsvParser.parseReemplazos(text)
      );
      ...
      localStorage.setItem('formulador_sub_cache_mp', JSON.stringify(productos));
      this._applyFilters();
  ...
  async _preloadFromBundledCsv() {
    const candidates = [
      new URL('../insumos_ref/mp-pt_mzr.csv', import.meta.url).toString(),
      new URL('./insumos_ref/mp-pt_mzr.csv', import.meta.url).toString(),
      new URL('../FORMULADOR%20-%20PROD.csv', window.location.href).toString(),
      new URL('../FORMULADOR - PROD.csv', window.location.href).toString(),
      new URL('/SGC_CALFERQUIM/insumos_ref/mp-pt_mzr.csv', window.location.origin).toString()
    ];
    ...
    if (Api.isConfigured()) {
      try {
        ...
        Utils.toast(`Catálogo cargado localmente; no se pudo sincronizar con Sheets: ${saveErr.message}`, 'warning', 6000);
      }
    } else {
      Utils.toast(`Catálogo cargado localmente con ${productos.length} productos`, 'success');
    }

    return productos;
  },

  _mergeCatalogs(primary = [], secondary = []) {
    const map = new Map();
    const push = row => {
      if (!row) return;
      const key = String(row.COD || row.ID_PROD || row.PRODUCTO || '').trim().toUpperCase();
      if (!key) return;
      if (!map.has(key)) {
        map.set(key, { ...row });
        return;
      ...
      for (const [field, value] of Object.entries(row)) {
        if (value !== '' && value !== null && value !== undefined) current[field] = value;
      }
    };
